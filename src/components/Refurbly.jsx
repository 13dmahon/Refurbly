...
export default function Refurbly({ onQuoteSaved, editingQuote, quotesCount, maxQuotes, onEditComplete }) {
  ...
  // Load pricing templates from Firestore
  ...
  const estimate = useMemo(() => {
    ...
    // PLASTERING: Use Firestore template if available
    ...
    // REWIRE: Use Firestore template if available
    ...
    // HEATING: Use Firestore template if available
    ...
    // WINDOWS: Use Firestore template if available
  ...
  const handleSaveQuote = async () => {
    ...
    if (!isEditing && quotesCount >= maxQuotes) {
      if (isPremium) {
        alert('You have reached the maximum of 10 saved quotes. Please delete some quotes to add new ones.');
      } else {
        alert('You have reached your free limit of 5 quotes. Upgrade to Premium to save more!');
      }
      return;
    }
